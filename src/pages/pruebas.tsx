...
const isHtml = html.includes('<table');
  ...
  return (
    <div className="w-full h-screen bg-gray-100">
        ...
        {/* div de abajo en el otro no existe */ }
  <div className="flex h-full w-full">

        {/* LISTA DE CHATS */}
        <aside
        className={`${isMobileChatOpen ? 'hidden' : 'flex'} lg:flex w-full lg:w-1/3 bg-white border-r relative flex-col`}
      >
        <div className="px-4 py-4 border-b flex justify-between items-center">
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => setShowMenu(!showMenu)}>
            <img src={user.picture} className="w-9 h-9 rounded-full" alt="avatar" />
            <span className="font-medium text-gray-800 text-sm truncate max-w-[120px]">{user.name}</span>
          </div>
          {showMenu && (
            <div className="absolute left-4 top-16 bg-white border rounded-lg shadow-md z-50 p-2">
              <button onClick={logout} className="text-sm text-gray-600 hover:text-red-500 transition font-medium">
                Cerrar sesión
              </button>
            </div>
          )}
